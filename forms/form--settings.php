<?php
    function yoft_settings_form() {
        if ( isset( $_POST[ 'assets_settings_nonce' ] ) ) {
            if ( ! wp_verify_nonce( sanitize_text_field( $_POST[ 'assets_settings_nonce' ] ), 'assets-settings-nonce' ) ) {
                ...
                if ( ! empty( $_POST[ 'bp_currency' ] ) ) {
                    update_option( 'bp_currency', sanitize_text_field( $_POST[ 'bp_currency' ] ) );
                } else {
                    update_option( 'bp_currency', '&euro;' );
                }
                if ( ! empty( $_POST[ 'bp_date_format' ] ) ) {
                    update_option( 'bp_date_format', sanitize_text_field( $_POST[ 'bp_date_format' ] ) );
                } else {
